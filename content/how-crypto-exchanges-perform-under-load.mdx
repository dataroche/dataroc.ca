...
summary: 'Part 2 in a series looking at the feasability of a real-time price aggregate of multiple crypto exchanges. In this entry, we indexed BTC, ETH and RPL over a period of 4 days incuding a high market activity period.'
---
...
Therefore, I assume the timestamps provided by my server are accurate to at least 5ms,
which means any latency measurement shows in this post should be considered accurate to
about 5ms.
...
can clearly see the 2 are strongly correlated. High trades throughput equals higher
insertion latency.
...
the connector latency is the delta between the server-provided timestamp of when trades
...
the increased load on my server. Since it's the receiving pipeline job to stamp the time
at which the message is received, any delay in processing artifically increases the
perceived delay of the connector.
