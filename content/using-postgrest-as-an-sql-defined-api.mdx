...
summary: 'Tutorial on how to get started with PostgREST (not Postgres): a high-performance REST API framework that sits on top of your PostgreSQL database.'
tags: 
  - 'dataroc.ca'
  - 'sql'
---
...
This post aims to summarize the different views currently exposed by the API and
how the schema defines those views. [In the next post of this
serie](/blog/inserting-data-via-the-postgrest-api-using-python), I highlight how the
...
`/api/portfolio_history?order=timestamp_s.asc`
...
The redacted symbol is not much on its own, since you can reasonably guess the pair
using the price and timestamp. That's a challenge to you the reader: decode one of the
redacted pairs I'm trading on!
