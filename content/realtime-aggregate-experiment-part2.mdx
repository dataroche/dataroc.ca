---
title: 'How different crypto exchanges websocket endpoints perform during high activity markets'
...
<StaticPlotlyChart filepath='rae-part2/active_connectors.json'>
    Figure 1: Active connectors per symbol during the experiment length. Each block is a connector that produced at least one message during an hour interval.
</StaticPlotlyChart>
...
<StaticPlotlyChart filepath='rae-part2/input_data_messages.json'>
    Figure 2: The total count of messages received in each 30 minutes interval. The trades_count are the trades streams and the spreads_count are the order book streams.
</StaticPlotlyChart>

## Database latency

The first clear indicator that the pipeline did not keep up is the database insertion
...
<StaticPlotlyChart filepath='rae-part2/database_latency.json'>
    Figure 3: The median (p50), 95th and 99th percentile of the insertion delay of records
    (both trade and order book streams). Note that these lines have been center-filtered 
    (e.g. using "future" data at time x) using a 10 buckets window. 
</StaticPlotlyChart>

### Database insertion latency is directly correlated with market activity
...
sound indicator of market volatility is actually a good thing...

If you superpose the trades count and the median database latency as in Figure 4, you can clearly see
the 2 are strongly correlated. High trades throughput equals higher insertion latency.

<StaticPlotlyChart filepath='rae-part2/database_latency_vs_trades.json'> 
    Figure 4: The median database insertion latency and the total trades count throughput.
    Notice how the trades spikes clearly correspond to a (smoothed) increase in the database
    latency.
</StaticPlotlyChart>

## Connector latency

This is the delta between the server-provided timestamp of when trades happened versus
the timestamp of when we actually receive the message. The data here is a bit harder to
interpret because the connector latency is indirectly affected by the increased load on
the my server. Since it's the receiving pipeline job to stamp the time at which the
message is received, any delay in processing artifically increases the perceived delay
of the connector.
...
<StaticPlotlyChart filepath='rae-part2/connector_latency_p50.json'>
    Figure 5: The median connector latency. Note that bitfinex was excluded due to an
    overall poor performance and abnormal latency peaks. 
</StaticPlotlyChart>

### Connector latency stability varies by exchange
...
difference around Aug 18, 13:00 - the high market activity event. Compare that to Kucoin
...
<StaticPlotlyChart filepath='rae-part2/connector_latency_p50_binance_kucoin.json'>
    ...
    Binance is stable while Kucoin performance varies in response to a higher activity
    market. 
</StaticPlotlyChart>
...
it's interesting to see that exchange performance data could be infered nonetheless.

I will soon share the code used to run these experiments. I'm currently refactoring the
pipeline to support a distributed execution. Moreover, I'm adding Redis as an in-memory,
high speed database for communications and an eventual real-time API.
